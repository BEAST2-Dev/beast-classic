...
public class FullyConjugateMultivariateTraitLikelihood extends IntegratedMultivariateTraitLikelihood { // implements ConjugateWishartStatisticsProvider {
    ...
    public FullyConjugateMultivariateTraitLikelihood(String traitName,
    	...
    	super(traitName, treeModel, diffusionModel, deltaParameter, deltaParameter, missingIndices, 
    			reciprocalRates, reciprocalRates, reciprocalRates, rateModel, null, rateModel, 
    			reciprocalRates, reciprocalRates);
    ...
    public FullyConjugateMultivariateTraitLikelihood(String traitName,
    	...
    	super(traitName, treeModel, diffusionModel, deltaParameter, deltaParameter, missingIndices, reciprocalRates, 
    			reciprocalRates, reciprocalRates, rateModel, driftModels, rateModel, reciprocalRates, reciprocalRates);
        //super(traitName, treeModel, diffusionModel, traitParameter, deltaParameter, missingIndices, cacheBranches, scaleByTime,
    ...
    public FullyConjugateMultivariateTraitLikelihood(String traitName,
                                                     ...
                                                     boolean reciprocalRates) {
    	super(traitName, treeModel, diffusionModel, deltaParameter, deltaParameter, missingIndices, reciprocalRates, 
    			reciprocalRates, reciprocalRates, strengthOfSelection, optimalValues, strengthOfSelection, 
    			reciprocalRates, reciprocalRates);
        //super(traitName, treeModel, diffusionModel, traitParameter, deltaParameter, missingIndices, cacheBranches, scaleByTime,
    ...
    public double getRescaledLengthToRoot(Node node) {

        double length = 0;

        if (!node.isRoot()) {
            Node parent = node.getParent();
            length += getRescaledBranchLengthForPrecision(node) + getRescaledLengthToRoot(parent);
        }
    ...
    protected double calculateAscertainmentCorrection(int taxonIndex) {
        ...
        Node tip = treeModel.getNode(taxonIndex);
        int nodeIndex = treeModel.getNode(taxonIndex).getNr();
...
//        System.err.println("root cprec    : " + upperPrecision);
        ...
        for (int i = 0; i < traitPrecision.length; ++i) {
            for (int j = 0; j < traitPrecision.length; ++j) {
                newPrec[i * traitPrecision.length + j] = traitPrecision[i][j] * upperPrecision;
            }
        }
