...
public class GeoSpatialCollectionModel { // extends AbstractModelLikelihood {
    ...
    public GeoSpatialCollectionModel(/*String name, */
    								RealParameter points,
                                     List<beast.geo.GeoSpatialDistribution> geoSpatialDistributions,
...
//    }
    ...
    public void handleVariableChangedEvent(int index) {
...
//    }
        ...
        for (int i = 0; i < dim; i++) {
            if (!validPointLogLikelihood[i]) {
                ...
                for (int j = 0; j < GeoSpatialDistribution.dimPoint; j++)
                    point[j] = points.getValue(offset + j);

                double pointLogLikelihood = 0;
                for (beast.geo.GeoSpatialDistribution distribution : geoSpatialDistributions) {
    ...
    private RealParameter points;
    private List<beast.geo.GeoSpatialDistribution> geoSpatialDistributions;
    private int dim;
