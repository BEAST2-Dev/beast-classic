...
public class WishartDistribution implements MultivariateDistribution {
    ...
    public WishartDistribution(double df, Double[] scaleMatrix) {
        this.df = df;
        this.dim = (int)Math.sqrt(scaleMatrix.length);
        
        this.scaleMatrix = new double[dim][dim];
        for (int i = 0; i < dim; i++) {
        	for (int j = 0; j < dim; j++) {
				this.scaleMatrix[i][j] = scaleMatrix[i * dim + j];
			}
        }

        SinvMat = new Matrix(this.scaleMatrix).inverse();
...
Expected output from main()"
 Wishart, df=2, scale = 500, PDF(1.0): -6.908755278962187
Gamma, shape = 1/1000, scale = 1000, PDF(1.0): -6.915086640662835
Wishart, df=4, scale = 5, PDF(1.0): -4.70517018596814
Gamma, shape = 1/1000, scale = 10, PDF(1.0): -4.7051701859880914
Wishart, uninformative, PDF(0.1): 2.3025850929940455
Wishart, uninformative, PDF(1.0): 0.0
Wishart, uninformative, PDF(10.0): -2.302585092994046
Testing new computations ...
Fast logPdf = -7.229278043324041
Slow logPdf = -7.229278043324041
*/